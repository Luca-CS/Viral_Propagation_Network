...
for index, row in cleaned_account.iterrows():
    ...
    dico[row.id_user] = {'total_views': 0,
                         ...
                         # 1 if the user already posted something, turns back to 0 after a while (so that the user can post again)
                         'posted': 0,
...
def action_number(poster, dico):
    ...
    followers = int(
        cleaned_account[cleaned_account['id_user'] == poster]['nb_followers'])
...
def choose(action_dic, rank_lst):
    ...
    choice_dic = {'view': [], 'like': [],
                  'comment': [], 'click': [], 'donate': []}
...
def update(dico, g):
    ...
    # dictionnaire avec id en clé et score en valeur
